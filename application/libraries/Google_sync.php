...
class Google_sync
{
    ...
    public function add_appointment(
        ...
        // Récupérer le nom du service (ou 'Unavailable' si absent)
        $serviceName = !empty($service['name']) ? $service['name'] : 'Unavailable';
    
        // Construire le résumé dans l'ordre : Client - Service
        $event->setSummary($clientName . ' - ' . $serviceName);
    
        // Construire la description en ajoutant les notes, le téléphone et l'email du client
        $notes = $appointment['notes'];
        $phone = !empty($customer['phone_number']) ? "Phone: " . $customer['phone_number'] : "";
        $email = !empty($customer['email']) ? "Email: " . $customer['email'] : "";
        if ($phone || $email) {
            // Ajoute un saut de ligne si des informations existent
            $notes .= "\n" . $phone . ($phone && $email ? "\n" : "") . $email;
        }
        ...
        $end->setDateTime(
            (new DateTime($appointment['end_datetime'], $timezone))->format(DateTimeInterface::RFC3339),
        );
        $event->setEnd($end);
        ...
        // Ajout de l'événement au calendrier Google.
    ...
    public function update_appointment(
        ...
        // Définir le résumé avec l'ordre souhaité : Client - Service
        $event->setSummary($clientName . ' - ' . $serviceName);
