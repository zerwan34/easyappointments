...
class Email_messages
{
    ...
    // --- TEST : Vérifier si le timezone du provider est reconnu par PHP ---
    if (!in_array($provider['timezone'], DateTimeZone::listIdentifiers())) {
        log_message(
            'error',
            'TIMEZONE INVALIDE détecté chez le provider (ID: ' . $provider['id'] . '), forcé vers Europe/Paris. ' .
            'Valeur actuelle = ' . $provider['timezone']
        );
