...
class Appointments_model extends EA_Model
{
    ...
    /**
     * Récupère les rendez-vous prévus entre deux dates
     */
    public function get_appointments_between($start_time, $end_time)
    {
        return $this->db
            ->select('id, start_datetime, end_datetime, id_users_customer, reminder_sent')
            ->where('start_datetime >=', $start_time)
            ->where('start_datetime <', $end_time)
            ->where('reminder_sent', 0) // Pour éviter de rappeler plusieurs fois
            ->get('appointments')
            ->result_array();
    }

    /**
     * Marque un rendez-vous comme rappel envoyé
     */
    public function mark_as_reminded($appointment_id)
    {
