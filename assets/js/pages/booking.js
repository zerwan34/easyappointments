    ...
    function initialize() {
        ...
        if (manageMode) {
            $(document).ready(function() {
                ...
                const alertHeight = isMobile ? '40%' : '15%';
                const buttonPadding = isMobile ? '20px' : '5px';
                const containerStyle = isMobile ?
                    "display: flex; justify-content: space-between; margin: 0 20px; gap: 20px;" :
                    "display: flex; justify-content: space-between; margin: 0 20px;";
    
                // Création d'un backdrop pour griser l'arrière-plan
                const backdrop = $('<div/>', {
                    id: 'modal-backdrop'
                }).css({
                    'position': 'fixed',
                ...
                // Ajout du backdrop au début du body
                $('body').prepend(backdrop);
    
                // Création de l'alerte en overlay avec message, question et deux boutons alignés
                const alertMessage = $('<div/>', {
                    class: 'alert alert-warning alert-dismissible text-center',
                    role: 'alert',
                    html: '⚠️ Vous êtes actuellement en train de modifier un rendez-vous existant. Veuillez vérifier vos modifications avant de valider.<br><br>' +
                          '<strong>Que souhaitez-vous faire ?</strong><br><br>' +
                          '<div style="' + containerStyle + '">' +
                              '<button id="new-appointment" type="button" class="btn btn-primary" style="padding:' + buttonPadding + ';">Prendre un rendez-vous</button>' +
                              '<button id="continue-modify" type="button" class="btn btn-secondary" style="padding:' + buttonPadding + ';">Modifier ce rendez-vous</button>' +
                          '</div>'
                }).css({
                    'position': 'fixed',
                    'top': '50%',
                ...
                // Bouton "Prendre un rendez-vous" : redirige vers BASE_URL
                alertMessage.find('#new-appointment').on('click', function() {
                    window.location.href = BASE_URL;
                });
    
                // Bouton "Modifier ce rendez-vous" : ferme l'alerte et retire le backdrop
                alertMessage.find('#continue-modify').on('click', function() {
                    alertMessage.fadeOut();
                    $('#modal-backdrop').fadeOut();
                });
    
                // Insertion de l'alerte dans le conteneur principal s'il existe, sinon dans le body
                const container = $('#wizard-frame-1');
                if (container.length) {
