...
App.Utils.CalendarDefaultView = (function () {
    ...
    function addEventListeners() {
         ...
         */
        $(document).on('click', '.edit-popover', (event) => {
            if ($popoverTarget) {
                $popoverTarget.popover('dispose');
         ...
         * @param {jQuery.Event} event
         */
        $(document).on('click', '.delete-popover', (event) => {
            if ($popoverTarget) {
                $popoverTarget.popover('dispose');
                $('#popover-overlay').remove();
            }
    ...
    function onEventClick(info) {
        ...
        if ($target.hasClass('fc-unavailability')) {
            ...
            $html = $('<div/>', {
                ...
                'html': [
                    ...
                    App.Utils.CalendarEventPopover.renderCustomContent(info),
                    $('<hr/>'),
                    // Boutons pour supprimer ou éditer (le bouton fermer a été déplacé)
                    $('<div/>', {
                        'class': 'd-flex justify-content-center',
                        'html': [
                            $('<button/>', {
        ...
        $target.popover({
            placement: 'top',
            container: 'body', 
            sanitize: false,   // autoriser l'HTML
            title: `
              <div class="d-flex justify-content-between align-items-start">
                <span>${App.Utils.String.escapeHtml(info.event.title)}</span>
                <button type="button" class="btn-close close-popover" aria-label="Close"></button>
              </div>
            `,
            content: $html,
            html: true,
        ...
        // Sélectionne la popover immédiatement après son apparition
        let $popoverElement = $('.popover');
        $popoverElement.css('z-index', 1060); // très élevé pour s'assurer qu'il est au-dessus
        
        // Ajoute maintenant l'overlay APRÈS avoir affiché la popover
        const $overlay = $('<div id="popover-overlay"></div>').css({
            position: 'fixed',
            top: 0,
            left: 0,
            width: '100%',
            ...
            zIndex: 1050 // inférieur à celui du popover
        });
        
        // L'overlay est ajouté directement au body
        $('body').append($overlay);
        
        // Évite immédiatement la fermeture en stoppant la propagation des événements
        setTimeout(() => {
          $overlay.on('click', (e) => {
            e.stopPropagation();
            if ($popoverTarget) {
        ...
        // Gérer la fermeture par la croix également
        $(document).off('click', '.close-popover').on('click', '.close-popover', (e) => {
          e.stopPropagation();
          if ($popoverTarget) {
            $popoverTarget.popover('dispose');
          }
        ...
        // Enregistre l'élément popover actuellement ouvert
        $popoverTarget = $target;
        lastFocusedEventData = info.event;
        
        // Fix popover position.
        $popoverElement = $('.popover'); // on récupère à nouveau la popover
        if ($popoverElement.length > 0 && $popoverElement.position().top < 200) {
            $popoverElement.css('top', '200px');
        }
    }        
